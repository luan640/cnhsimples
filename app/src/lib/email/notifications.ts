...
export async function sendStudentLessonConfirmedEmail(params: {
  ...
  return sendEmail({
    to: params.to,
    subject: receiptUrl ? 'Sua aula foi registrada — comprovante disponível' : 'Sua aula foi confirmada',
    text:
      `Ola, ${studentName}. O instrutor ${instructorName} confirmou a realizacao da sua aula ` +
      `de ${lessonDate} as ${lessonTime}. ` +
      receiptSection +
      `Acesse suas aulas: ${lessonsUrl}`,
    html: `
      <div style="font-family: Arial, sans-serif; color: #0F172A; line-height: 1.6;">
        <h1 style="margin-bottom: 12px;">Aula confirmada</h1>
        <p>Ola, ${studentName}.</p>
        <p>
          O instrutor <strong>${instructorName}</strong> confirmou a realizacao da sua aula
          do dia <strong>${lessonDate}</strong> as <strong>${lessonTime}</strong>.
        </p>
        ${receiptUrl ? '<p>O comprovante de registro foi anexado e esta disponivel para consulta.</p>' : ''}
