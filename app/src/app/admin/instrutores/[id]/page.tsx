...
import { getInstructorDetail } from '@/lib/admin/queries'
...
export default async function InstructorDetailPage({
  ...
  const result = await getInstructorDetail(id)
  if (!result) notFound()
    ...
    <div className="p-6 space-y-6 max-w-4xl">
      ...
      <div className="flex items-start gap-4 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-bold" style={{ color: '#0F172A' }}>{profile.full_name}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          >
            <p className="text-sm font-semibold mb-4" style={{ color: '#0F172A' }}>Dados Pessoais</p>
            <div className="grid grid-cols-2 gap-4">
              <Row label="CPF" value={profile.cpf} />
              <Row label="Telefone" value={profile.phone} />
              <Row label="CEP" value={profile.cep} />
              <Row label="Cidade / Estado" value={profile.city ? `${profile.city} / ${profile.state ?? 'CE'}` : null} />
              <Row label="Bairro" value={profile.neighborhood} />
              <Row label="Endereço" value={profile.street ? `${profile.street}, ${profile.number ?? 'S/N'}` : null} />
            </div>
          </div>
          ...
          <div
            ...
            style={{ background: '#fff', borderColor: '#E2E8F0' }}
          >
            <p className="text-sm font-semibold mb-4" style={{ color: '#0F172A' }}>Dados Profissionais</p>
            <div className="grid grid-cols-2 gap-4">
              <Row label="Categoria CNH" value={profile.category} />
              <Row label="Número da CNH" value={profile.cnh_number} />
              <Row label="Validade CNH" value={profile.cnh_expires_at ? new Date(profile.cnh_expires_at).toLocaleDateString('pt-BR') : null} />
              <Row label="Credencial DETRAN" value={profile.detran_credential_number} />
              <Row label="Validade Credencial" value={profile.detran_credential_expires_at ? new Date(profile.detran_credential_expires_at).toLocaleDateString('pt-BR') : null} />
              <Row label="Experiência (anos)" value={profile.experience_years} />
              <Row label="Valor/hora" value={profile.hourly_rate ? `R$ ${Number(profile.hourly_rate).toFixed(2)}` : null} />
              <Row label="Raio de atendimento" value={profile.service_radius_km ? `${profile.service_radius_km} km` : null} />
            </div>
            {profile.bio && (
              <div className="mt-4">
                <span className="text-xs font-medium block mb-1" style={{ color: '#94A3B8' }}>Bio</span>
                <p className="text-sm" style={{ color: '#0F172A' }}>{profile.bio}</p>
              </div>
            )}
          </div>
          ...
          <div
            ...
            style={{ background: '#fff', borderColor: '#E2E8F0' }}
          >
            <p className="text-sm font-semibold mb-4" style={{ color: '#0F172A' }}>Dados para Saque (PIX)</p>
            <div className="grid grid-cols-2 gap-4">
          ...
          <div
            ...
            style={{ background: '#fff', borderColor: '#E2E8F0' }}
          >
            <p className="text-sm font-semibold mb-4" style={{ color: '#0F172A' }}>Documentos enviados</p>
            {documents.length === 0 ? (
              <p className="text-sm" style={{ color: '#94A3B8' }}>Nenhum documento enviado.</p>
            ) : (
        ...
        <div className="space-y-4">
          ...
          {profile.rejection_reason && (
            ...
            >
              <p className="text-xs font-semibold mb-1" style={{ color: '#DC2626' }}>Motivo da rejeição</p>
              <p className="text-sm" style={{ color: '#DC2626' }}>{profile.rejection_reason}</p>
            </div>
          )}
        </div>
      </div>
